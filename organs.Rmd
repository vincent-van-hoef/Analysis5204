...
The full list of protein-wise Pearson correlation results for the different organs is collected in excel files in **Results/Plasma/Organ/[organ]/[organ]_pearson_correlation.xlsx**. All individuals regressions are plotted and collected in the subfolder **regression_plots/**. 

The results are also shown here in an interactive table. Results are ranked by adjusted p-value and a link to the corresponding plot is added. 
...
create_organ_table <- function(source = "Plasma", organ = "general") {
  ...
  mutate("plot" = paste0('<a href ="./../Results/', source, '/Organ/', organ, '/regression_plots/', Assay, '_regression.png"  target="_blank">', Assay, '</a>')) %>%
...
The full list of protein-wise Pearson correlation results for the different organs is collected in excel files in **Results/Serum/Organ/[organ]/[organ]_pearson_correlation.xlsx**. All individuals regressions are plotted and collected in the subfolder **regression_plots/**. 
